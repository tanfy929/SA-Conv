...
class Mainnet(nn.Module):
    def __init__(self, args):
        ...
        self.xnet = Xnet(self.num_Z+3, args)                                  # 3 means R,G,B channels for color image
        ...
        self.fxnet = Xnet(self.num_Z + 3, args)
    ...
    def make_xnet(self, iters, channel):
        ...
        for i in range(iters):
            layers.append(Xnet(channel,self.args))
    ...
    def forward(self, input):
        # save mid-updating results
         ListB = []
         ListCM = []
        #  print('input', input.shape) # ([16, 3, 64, 64])
...
class Xnet(nn.Module):
    def __init__(self, channels, args):
        ...
        self.headx = nn.ModuleList([fcnn.Fconv_PCA(kernel_size, channels, n_feats, tranNum, inP=kernel_size, padding=(kernel_size-1)//2, ifIni=1, Smooth=Smooth, iniScale=1.0)])
