...
try:
    ...
    try:
        ...
        def _set_worker_signal_handlers(*args, **kwargs): pass
    ...
    def _put_indices(self):
        ...
        if self.num_workers > 0:
            try:
                ...
                return
            self.index_queues[self.worker_queue_idx].put((self.send_idx, batch_indices))
    ...
    def _get_batch(self):
        ...
        if self.num_workers == 0:
            ...
            try:
                ...
                samples = self.collate_fn([self.dataset[i] for i in batch_indices])
                samples.append(idx_scale)
                return samples
            except Exception:
                raise
        ...
        while True:
            ...
            if batch is None:
                # 只有当 loop 出现严重问题时才会发生
                raise RuntimeError('DataLoader worker died unexpectedly')
