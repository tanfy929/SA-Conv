...
#        iniw = torch.randn(outNum, inNum, self.expand, self.Basis.size(3))*0.03
        iniw = Getini_reg(Basis.size(3), inNum, outNum, self.expand, weight)
...
class FCNN_complex(nn.Module):
    # è¿éX,Y,Mask,é½æ¯ 1*1*P*Pç?
    #ç¨æ¥å¾å°åºåº
    def __init__(self,  sizeP, inNum, outNum, tranNum=8, inP = None, padding=None, ifIni=0, bias=True):
        super(FCNN_complex,self).__init__()
        
        if inP==None:
            inP = sizeP
            
        inBasisC, inBasisS = GetBasis_plus(sizeP, tranNum=tranNum, inP=inP)
        BasisC = nn.Parameter(inBasisC,requires_grad=False).cuda()
        BasisS = nn.Parameter(inBasisS,requires_grad=False).cuda()
        if ifIni:
            iniA, iniB = Getini(inP,1, int((inNum*outNum+1)//2))
            a = nn.Parameter(iniA, requires_grad=True).cuda()
            b = nn.Parameter(iniB, requires_grad=True).cuda()
            tempw1 = (torch.einsum('ijok,mnk->monij', BasisC, a)+torch.einsum('ijok,mnk->monij', BasisS, b)).reshape([-1,1,tranNum, sizeP, sizeP])
            tempw2 = (torch.einsum('ijok,mnk->monij', BasisC, b)+torch.einsum('ijok,mnk->monij', -BasisS, a)).reshape([-1,1,tranNum, sizeP, sizeP])
            tempW = torch.cat((tempw1, tempw2), 1).reshape(-1,tranNum,sizeP,sizeP)[0:inNum*outNum,...]
            self.w = tempW.reshape([outNum, inNum, tranNum, sizeP, sizeP]).permute([0,2,1,3,4]).reshape([-1,inNum, sizeP, sizeP])
        else:
            iniA, iniB = Getini(inP,1, int((inNum*outNum*tranNum+1)//2))
            a = nn.Parameter(iniA, requires_grad=True).cuda()
            b = nn.Parameter(iniB, requires_grad=True).cuda()
            tempw1 = (torch.einsum('ijok,mnk->monij', BasisC, a)+torch.einsum('ijok,mnk->monij', BasisS, b)).reshape([-1,1,tranNum, sizeP, sizeP])
            tempw2 = (torch.einsum('ijok,mnk->monij', BasisC, b)+torch.einsum('ijok,mnk->monij', -BasisS, a)).reshape([-1,1,tranNum, sizeP, sizeP])
            tempW = torch.cat((tempw1, tempw2), 1).reshape(-1,tranNum,sizeP,sizeP)[0:inNum*outNum*tranNum,...]
            tempW = tempW.reshape([outNum, tranNum, inNum, tranNum, sizeP, sizeP]).permute([0,3,2,1,4,5])
            for i in range(tranNum):
                ind = np.hstack((np.arange(tranNum-i,tranNum), np.arange(tranNum-i) ))
                tempW[:,i,:,:,...] = tempW[:,i,:,ind,...]
            self.w = tempW.reshape([outNum*tranNum, inNum*tranNum, sizeP, sizeP])
#	testY = [int(test[i]) for i in range(num_test) if (i+1)%785 == 0]
        c = nn.Parameter(torch.zeros(1,outNum,1,1), requires_grad=True).cuda()
        if bias:
            self.c = c.repeat([1,1,tranNum,1]).reshape([1,outNum*tranNum,1,1])
        else:
            self.c = 0
        if padding == None:
            padding = (sizeP-1)//2
        self.p = padding
    def forward(self, X):
        X               = F.conv2d(X, self.w, stride =1, padding = int(self.p), groups=1, dilation =1)##
        X               = X+self.c
        return X    
...
#def Getini_reg(nNum, inNum, outNum): 
##    A = (np.random.rand(outNum,inNum,nNum)-0.5)*2*2.4495/np.sqrt((inNum)*nNum)
##    A = np.random.randn(outNum,inNum,nNum)*2.4495/np.sqrt(inNum*nNum*25)
#    A = np.random.randn(outNum,inNum,nNum)*0.03*
...
#def Getini_PCA(nNum, inNum, outNum, weight): 
#    A = np.random.randn(outNum,inNum,nNum)*0.03
#    return torch.FloatTensor(A)
