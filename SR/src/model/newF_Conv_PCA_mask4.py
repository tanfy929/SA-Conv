...
class GetBasis(nn.Module):
    def __init__(self, sizeP, tranNum=8, inP=None):
        ...
        self.k = torch.arange(-(inP//2),inP//2+1, device=self.device).reshape(1,1,1,inP,1)*v
        self.l = torch.arange(-(inP//2),inP//2+1, device=self.device).reshape(1,1,1,1,inP)*v
    ...
    def forward(self):
        ...
        Mask_ = MaskC(self.sizeP, self.tranNum, self.theta0, self.Cx, self.Cy, self.device)
        Mask = Mask_.detach().reshape(self.sizeP,self.sizeP,1,1,1) # 不影响梯度
        ...
        # print('BasisR', BasisR.shape) # ([5, 5, 8, 50])
        # print('U',self.U.shape) # ([50, 25])
        ...
        # with open('log_newfconv2.txt', 'a', encoding='utf-8') as f:
        #     f.write('############ BasisR ############' + str(BasisR) + '\n') 
        return BasisR
...
def MaskC(SizeP, tranNum, theta0, Cx, Cy, device_):
        p = (SizeP-1)/2

        x = torch.arange(-p, p + 1, dtype=torch.float32, device=device_) / p
...
class F_BN(nn.Module):
    def __init__(self,channels, tranNum=8, affine=True, track_running_stats: bool = True):
