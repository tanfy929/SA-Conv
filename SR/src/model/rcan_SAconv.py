...
class RCAB(nn.Module):
    def __init__(
        ...
        bias=True, bn=False, act=fn.F_relu(True), res_scale=1.0, iniScale=1.0):
        ...
        for i in range(2):
            ...
            if bn: modules_body.append(fn.F_BN(n_feat))
...
class ResidualGroup(nn.Module):
    def __init__(self, tranNum, inP, Smooth, n_feat, kernel_size, reduction, act, res_scale, n_resblocks, iniScale=1.0):
        ...
        modules_body = []
        modules_body = [
            RCAB(tranNum, inP, Smooth, n_feat, kernel_size, reduction, bias=True, bn=False, act=fn.F_relu(True), res_scale=1.0, iniScale = iniScale) \
...
class RCAN_plus(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.out = fn.Fconv_PCA_out(kernel_size,n_feats,n_feats*tranNum,tranNum,inP=inP,padding=(kernel_size-1)//2,  Smooth = Smooth, iniScale = iniScale)
        modules_tail = [
    ...
    def forward(self, x):
        ...
        res_out = self.out(res, Cx, Cy, theta0)
        x = self.tail(res_out)
        x = self.add_mean(x)
