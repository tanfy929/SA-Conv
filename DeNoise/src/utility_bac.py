...
class checkpoint():
    def __init__(self, args):
        ...
        if not args.load:
            if not args.save:
                args.save = now
            self.dir = os.path.join('..', 'experiment_no_argument', args.save) 
            #self.dir = os.path.join('..', 'experiment', args.save)
        else:
            self.dir = os.path.join('..', 'experiment_no_argument', args.load)
...
#            os.system('rm -rf ' + self.dir)
    ...
    def save(self, trainer, epoch, is_best=False):
        ...
        torch.save(self.ssim, self.get_path('ssim_log.pt'))

    def add_log(self, log):
        self.log = torch.cat([self.log, log])
    def add_ssim(self, ssim):
        self.ssim = torch.cat([self.ssim, ssim])
    ...
    def save_results(self, dataset, filename, save_list, scale):
        if self.args.save_results:
            ...
            for v, p in zip(save_list, postfix):
                ...
                io.imsave(('{}{}.png'.format(filename, p)), tensor_cpu)
                if self.args.save_mat:
                    sio.savemat(('{}{}.mat'.format(filename, p)), {'X':tensor_cpu})    
