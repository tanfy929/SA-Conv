    ...
    def train(self):
        ...
        for batch, (lr, hr, _,) in enumerate(self.loader_train):
            ...
            # for layer in self.model.modules():
                # print("####", layer)
            # print("###Models", self.model.modules)
            if self.args.if_filter_decay:
                filter_regularization = self.args.filter_decay * filter_reg(self.model, False)
            else: 
                filter_regularization = 0

            loss += filter_regularization
#            print(torch.mean(hr))
            loss.backward()
...
#                    print(torch.mean(hr))
                    ...
                    if self.args.save_results:
                        ...
                        # self.ckp.write_log('[{}], PSNR: {:.4f}, SSIM: {:.4f}'.format(filename, temp_psnr, temp_ssim))
...
#            self.ckp.end_background()
    ...
    # def filter_reg(model, L1_reg = False): # calculate filter regularization
    #     reg = 0
    #     if L1_reg:
    #         for layer in self.model.modules():
    #             if isinstance(layer, fn.Fconv_PCA) or isinstance(layer, fn.Fconv_PCA_out):
    #                 reg += torch.norm(layer.decay_weights, 1)
    #     else:
    #         print(11111)
    #         for layer in self.model.modules():
    #             if isinstance(layer, fn.Fconv_PCA) or isinstance(layer, fn.Fconv_PCA_out):
    #                 reg += (layer.decay_weights**2).sum()
    #     return reg
